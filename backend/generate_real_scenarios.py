...
ATTACKS = [
    ...
    # === Additional malicious scenarios ===
    {
        "package": "coa", "registry": "npm",
        "difficulty": "easy", "is_malicious": True,
        "attack_name": "coa npm Account Compromise",
        "attack_type": "account_hijack",
        "version_from": "2.0.2", "version_to": "2.0.3",
        "real_cve": None, "real_cvss": 9.8,
        "identity_overrides": {"flags": ["Maintainer account credentials likely compromised"]},
        "timing_overrides": {"cadence_normal": False, "flags": ["5 versions published in rapid succession within hours", "Release after 2+ year dormancy"]},
        "shape_overrides": {"files_added": ["compile.js", "sdd.dll"], "flags": ["New file 'compile.js' with obfuscated code", "Windows DLL bundled in npm package"]},
        "behavior_overrides": {"categories": {"compute": "red", "file_io": "red", "network": "red", "install_scripts": "red", "obfuscation": "red"}, "flags": ["Preinstall script executes obfuscated code", "Downloads platform-specific payload", "Deploys credential stealer"]},
        "flow_overrides": {"outbound_connections": [{"domain": "pastorcryptograph.at", "type": "payload_download"}], "data_reads": ["system credentials", "browser data"], "flags": ["Downloads executable from unknown domain", "Reads browser credential stores"]},
        "context_overrides": {"update_summary": "Patch version adds preinstall script downloading and executing obfuscated payloads", "mismatch_score": 0.98, "flags": ["Command-line argument parser downloading executables"]},
        "postmortem": "The coa package maintainer's npm account was hijacked. Five malicious versions were published within hours after years of dormancy. The preinstall script downloaded credential-stealing malware. Broke React builds globally since coa is a transitive dependency of react-scripts. Fixed within hours.",
    },
    {
        "package": "rc", "registry": "npm",
        "difficulty": "medium", "is_malicious": True,
        "attack_name": "rc npm Account Compromise",
        "attack_type": "account_hijack",
        "version_from": "1.2.8", "version_to": "2.3.9",
        "real_cve": None, "real_cvss": 9.8,
        "identity_overrides": {"flags": ["Maintainer account credentials likely compromised"]},
        "timing_overrides": {"cadence_normal": False, "flags": ["Major version jump 1.x → 2.x without changelog", "Multiple versions published within minutes"]},
        "shape_overrides": {"flags": ["Massive version jump from 1.2.8 to 2.3.9"]},
        "behavior_overrides": {"categories": {"compute": "red", "file_io": "red", "network": "red", "install_scripts": "red", "obfuscation": "red"}, "flags": ["Obfuscated preinstall script", "Downloads and executes remote code", "Same payload pattern as coa compromise"]},
        "flow_overrides": {"outbound_connections": [{"domain": "pastorcryptograph.at", "type": "payload_download"}], "data_reads": ["system credentials", "browser data"], "flags": ["Same C2 domain as coa attack"]},
        "context_overrides": {"update_summary": "Unexpected major version jump with obfuscated preinstall payload", "mismatch_score": 0.98, "flags": ["Config loader jumping 2 major versions with malicious preinstall"]},
        "postmortem": "Part of the same campaign as the coa compromise — same attacker, same payload domain. The rc package provides runtime configuration loading for Node apps. The version jump from 1.2.8 to 2.3.9 was itself an anomaly signal. Caught within hours due to build breakages.",
    },
    {
        "package": "eslint-scope", "registry": "npm",
        "difficulty": "hard", "is_malicious": True,
        "attack_name": "eslint-scope Credential Theft",
        "attack_type": "account_hijack",
        "version_from": "3.7.1", "version_to": "3.7.2",
        "real_cve": None, "real_cvss": 9.8,
        "identity_overrides": {"flags": ["Legitimate maintainer's credentials compromised via phishing"]},
        "timing_overrides": {"cadence_normal": False, "flags": ["Published outside normal release schedule"]},
        "shape_overrides": {"files_added": ["build.js"], "flags": ["New build.js not present in GitHub source"]},
        "behavior_overrides": {"categories": {"compute": "green", "file_io": "red", "network": "red", "install_scripts": "red", "obfuscation": "yellow"}, "flags": ["Postinstall reads .npmrc token", "Sends npm auth tokens to attacker server", "Targets all users who install this version"]},
        "flow_overrides": {"outbound_connections": [{"domain": "sstatic1.histats.com", "type": "data_exfiltration"}], "data_reads": [".npmrc auth tokens", "npm registry tokens"], "flags": ["Exfiltrates npm authentication tokens", "Disguised as analytics endpoint"]},
        "context_overrides": {"update_summary": "Patch adds postinstall script that reads and exfiltrates .npmrc authentication tokens", "mismatch_score": 0.95, "flags": ["Scope analysis library stealing npm credentials"]},
        "postmortem": "Attacker phished credentials of an eslint-scope maintainer and published 3.7.2 with a postinstall script that harvested .npmrc tokens — a supply chain attack designed to steal credentials of every developer who installed it, enabling further attacks. npm revoked all affected tokens.",
    },
    {
        "package": "rest-client", "registry": "pypi",
        "difficulty": "medium", "is_malicious": True,
        "attack_name": "rest-client PyPI Backdoor",
        "attack_type": "typosquat",
        "version_from": "2.0.0", "version_to": "2.0.1",
        "real_cve": None, "real_cvss": 7.5,
        "identity_overrides": {"is_usual_publisher": True, "flags": ["Package existed for 2+ years before backdoor added"]},
        "timing_overrides": {"flags": ["Long period of inactivity before this release"]},
        "shape_overrides": {"flags": ["setup.py modified with obfuscated install hook"]},
        "behavior_overrides": {"categories": {"compute": "red", "file_io": "red", "network": "red", "install_scripts": "red", "obfuscation": "red"}, "flags": ["setup.py contains base64-encoded payload", "Collects system information on install", "Exfiltrates environment variables"]},
        "flow_overrides": {"outbound_connections": [{"domain": "68.65.122.245", "type": "data_exfiltration"}], "data_reads": ["environment variables", "system info", "hostname"], "flags": ["Sends data to hardcoded IP address", "Collects environment variables including secrets"]},
        "context_overrides": {"update_summary": "setup.py now contains obfuscated code that collects and exfiltrates system information and environment variables", "mismatch_score": 0.92, "flags": ["REST client library collecting and transmitting system data on install"]},
        "postmortem": "The rest-client package on PyPI was backdoored with a setup.py payload that collected environment variables (including cloud credentials and API keys) and sent them to a hardcoded IP. The package had been dormant, making the release itself an anomaly. Discovered by a security researcher reviewing install hooks.",
    },
    # === Additional benign scenarios ===
    {
        "package": "chalk", "registry": "npm",
        "difficulty": "easy", "is_malicious": False,
        "attack_name": None, "attack_type": None,
        "version_from": "5.3.0", "version_to": "5.4.0",
        "real_cve": None, "real_cvss": None,
        "identity_overrides": {},
        "timing_overrides": {},
        "shape_overrides": {},
        "behavior_overrides": {"categories": {"compute": "green", "file_io": "green", "network": "green", "install_scripts": "green"}},
        "flow_overrides": {},
        "context_overrides": {"update_summary": "Minor feature update adding new color methods", "mismatch_score": 0.0},
        "postmortem": "Routine feature update from the established maintainer (sindresorhus). No anomalies in any dimension.",
    },
    {
        "package": "debug", "registry": "npm",
        "difficulty": "easy", "is_malicious": False,
        "attack_name": None, "attack_type": None,
        "version_from": "4.3.4", "version_to": "4.3.5",
        "real_cve": None, "real_cvss": None,
        "identity_overrides": {},
        "timing_overrides": {},
        "shape_overrides": {},
        "behavior_overrides": {"categories": {"compute": "green", "file_io": "green", "network": "green", "install_scripts": "green"}},
        "flow_overrides": {},
        "context_overrides": {"update_summary": "Patch release with minor bug fixes", "mismatch_score": 0.0},
        "postmortem": "Standard patch release. Debugging utility that accesses environment variables for DEBUG flag — this is expected behavior, not exfiltration.",
    },
    {
        "package": "flask", "registry": "pypi",
        "difficulty": "easy", "is_malicious": False,
        "attack_name": None, "attack_type": None,
        "version_from": "3.0.0", "version_to": "3.0.1",
        "real_cve": None, "real_cvss": None,
        "identity_overrides": {},
        "timing_overrides": {},
        "shape_overrides": {},
        "behavior_overrides": {"categories": {"compute": "green", "file_io": "green", "network": "green", "install_scripts": "green"}},
        "flow_overrides": {},
        "context_overrides": {"update_summary": "Patch release with bug fixes for request handling", "mismatch_score": 0.0},
        "postmortem": "Routine patch from the Pallets team. Flask is a web framework so network activity is inherently expected. No anomalies.",
    },
    {
        "package": "semver", "registry": "npm",
        "difficulty": "easy", "is_malicious": False,
        "attack_name": None, "attack_type": None,
        "version_from": "7.5.4", "version_to": "7.6.0",
        "real_cve": None, "real_cvss": None,
        "identity_overrides": {},
        "timing_overrides": {},
        "shape_overrides": {},
        "behavior_overrides": {"categories": {"compute": "green", "file_io": "green", "network": "green", "install_scripts": "green"}},
        "flow_overrides": {},
        "context_overrides": {"update_summary": "Minor version with improved version range parsing", "mismatch_score": 0.0},
        "postmortem": "Standard minor release from npm's own semver library. Well-maintained by the npm team. No anomalies.",
    },
]
