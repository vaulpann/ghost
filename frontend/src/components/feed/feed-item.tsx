    ...
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 sm:gap-2.5 mb-1 sm:mb-1.5 flex-wrap">
          <span className="font-medium text-[13px] text-foreground group-hover:text-foreground transition-colors">
            {item.package_name}
          </span>
          <RegistryBadge registry={item.package_registry} />
          <span className="text-[11px] text-muted-foreground/50 font-mono">{item.version_string}</span>
        </div>
        <p className="text-[12px] text-muted-foreground/80 truncate leading-relaxed">
