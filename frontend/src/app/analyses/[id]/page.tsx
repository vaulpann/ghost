...
export default function AnalysisDetailPage() {
  ...
  if (loading || !analysis) {
    return <div className="text-muted-foreground/50 text-sm">Loading analysis...</div>;
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="animate-fade-in">
        <Link href="/analyses" className="text-[12px] text-muted-foreground/60 hover:text-foreground/50 transition-colors">
          &larr; Analyses
        </Link>
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mt-3">
          ...
          <div className="flex sm:flex-col gap-3 sm:gap-1 sm:text-right text-[11px] text-muted-foreground/50">
            <p>Status: <span className="text-foreground/50">{analysis.status}</span></p>
            {analysis.total_cost_usd !== null && (
              <p>Cost: <span className="text-foreground/50 font-mono">${analysis.total_cost_usd.toFixed(4)}</span></p>
            )}
      ...
      {analysis.summary && (
        <div className="rounded-2xl glass p-6 animate-fade-in animate-fade-in-delay-1">
          <p className="text-[11px] text-muted-foreground/60 uppercase tracking-wider font-medium mb-3">Executive Summary</p>
          <p className="text-[14px] text-foreground/60 leading-relaxed">{analysis.summary}</p>
          {synthesisResult?.recommended_action && (
            <div className="mt-4 pt-4 border-t border-border">
              <p className="text-[11px] text-muted-foreground/60 uppercase tracking-wider font-medium mb-1">Recommended Action</p>
              <p className="text-[13px] text-foreground/50">{synthesisResult.recommended_action.replace(/_/g, " ")}</p>
            </div>
          )}
        </div>
      )}
          ...
          {(["report", "findings", "diff", "raw"] as const).map((tab) => (
            ...
            >
              ...
              {tab === "findings" && findings.length > 0 && (
                <span className="ml-1.5 text-[10px] text-muted-foreground/60">{findings.length}</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="animate-fade-in animate-fade-in-delay-3">
          ...
          <div className="space-y-4">
            {synthesisResult?.detailed_report ? (
              <div className="rounded-2xl glass p-8 prose-ghost">
                <Markdown
                  components={{
                    h1: ({ children }) => <h1 className="text-xl font-semibold text-foreground mb-4 mt-0">{children}</h1>,
                    ...
                    code: ({ children }) => (
                      <code className="text-[12px] font-mono bg-foreground/[0.04] text-emerald-400/70 px-1.5 py-0.5 rounded-md border border-border">
                        {children}
                      </code>
                    ),
                    pre: ({ children }) => (
                      <pre className="bg-foreground/[0.02] border border-border rounded-lg p-4 overflow-x-auto mb-4">
                        {children}
                      </pre>
                    ),
                    blockquote: ({ children }) => (
                      <blockquote className="border-l-2 border-emerald-500/30 pl-4 my-4 text-muted-foreground italic">
                        {children}
                      </blockquote>
                    ),
              ...
              <p className="text-muted-foreground/50 text-sm">No detailed report available.</p>
            )}
          </div>
        )}
        ...
        {activeTab === "findings" && (
          <div className="space-y-3">
            {findings.length === 0 ? (
              <p className="text-muted-foreground/50 text-sm">No findings.</p>
            ) : (
              findings.map((f, i) => (
            ...
            <pre className="p-5 text-[12px] font-mono overflow-x-auto max-h-[700px] overflow-y-auto leading-relaxed">
              {diff ? (
                diff.split("\n").map((line, i) => (
                  ...
                  >
                    <span className="text-muted-foreground/30 select-none inline-block w-10 text-right mr-4 text-[10px]">
                      {i + 1}
                    </span>
                ...
                <span className="text-muted-foreground/50">No diff available.</span>
              )}
            </pre>
          </div>
        )}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 p-4">
        ...
        <span className="text-[10px] text-muted-foreground/50 font-mono uppercase tracking-wider">{finding.category}</span>
        <span className="font-medium text-[13px] text-foreground/70 flex-1">{finding.title}</span>
        <span className="text-[11px] text-muted-foreground/50 tabular-nums">
          {(finding.confidence * 100).toFixed(0)}%
        </span>
        <svg
          className={cn("w-4 h-4 text-muted-foreground/30 transition-transform", expanded && "rotate-180")}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3 border-t border-border/50 pt-3">
          ...
          {finding.evidence?.items?.map((ev, i) => (
            <div key={i} className="rounded-lg bg-foreground/[0.02] border border-border p-3">
              <p className="text-[11px] text-muted-foreground/60 font-mono mb-2">
                {ev.file_path} <span className="text-muted-foreground/30">L{ev.line_start}–{ev.line_end}</span>
              </p>
              <pre className="text-[12px] font-mono text-foreground/50 overflow-x-auto">{ev.snippet}</pre>
              <p className="text-[11px] text-muted-foreground/70 mt-2">{ev.explanation}</p>
            </div>
          ))}

          {finding.mitre_technique && (
            <p className="text-[11px] text-muted-foreground/50">MITRE ATT&CK: <span className="text-muted-foreground">{finding.mitre_technique}</span></p>
          )}
          {finding.remediation && (
            <p className="text-[11px] text-muted-foreground/50">Remediation: <span className="text-muted-foreground">{finding.remediation}</span></p>
          )}
        </div>
      )}
    </div>
...
function CollapsibleJSON({ title, data }: { title: string; data: any }) {
  ...
  return (
    <div className="rounded-xl glass overflow-hidden">
      <button
        onClick={() => setExpanded(!expanded)}
        className="w-full px-5 py-3.5 text-left text-[13px] font-medium text-foreground/50 flex items-center justify-between hover:bg-foreground/[0.02] transition-colors"
      >
        {title}
        <span className="text-[11px] text-muted-foreground/50">{expanded ? "collapse" : "expand"}</span>
      </button>
      {expanded && (
        <pre className="border-t border-border p-5 text-[11px] font-mono text-muted-foreground/80 overflow-x-auto max-h-96 overflow-y-auto leading-relaxed">
